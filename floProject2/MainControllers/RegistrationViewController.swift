...
class RegistrationViewController: UIViewController {
    ...
    @objc func createTapped(){
        ...
        let period = createPeriod(from: dateOfPeriodPicker.date)
        let userInfo : UserInfo = UserInfo(login: LocalStorageManager.getUser()[0].login, password: LocalStorageManager.getUser()[0].password, name: nameTextField.text!, email: emailTextField.text!, dateOfBirth: dateOfBirthPicker.date, dateOfPeriod: dateOfPeriodPicker.date,period: [period])
