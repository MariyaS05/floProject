...
class RegistrationViewController: UIViewController {
    ...
    let nameTextField =  GFTextField(placeholder: "Ваше имя", textContentType: .name, secure: false)
    let emailTextField =  GFTextField(placeholder: "E-mail", textContentType: .emailAddress, secure: false)
    var dateOfBirthPicker =  UIDatePicker()
    var dateOfPeriodPicker =  UIDatePicker()
    let loginLabel =  GFTitleLabel(fontSize: 24, weight: .heavy, textAlignment: .center)
    let dateOfBirthLabel =  GFTitleLabel(fontSize: 20, weight: .medium, textAlignment: .left)
    let dateOfPeriodLabel =  GFTitleLabel(fontSize: 20, weight: .medium, textAlignment: .left)
    let buttonCreateAnAccount = GFButton(title: "Зарегистироваться", textColor: .white)
   ...
   private func configure(){
    ...
    @objc func createTapped(){
        ...
        print(LocalStorageManager.getUserInfo().first )
        print(LocalStorageManager.getUserInfo().count)
